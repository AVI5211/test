# vychron EKS Deployment Guide — test
...
- **GitOps repo** — `AVI5211/AVI5211-gitops-config` contains your Helm chart and ArgoCD manifests
...
```json
{
  ...
  "Statement": [{
    ...
    "Condition": {
      "StringLike": {
        "token.actions.githubusercontent.com:sub": "repo:AVI5211/test:ref:refs/heads/main"
      },
      "StringEquals": {
...
This trust policy only allows the `main` branch of **AVI5211/test** to assume the role.
...
ECR login → docker build → docker push (tag: sha-<commit>)
  ↓
Clone AVI5211/AVI5211-gitops-config
  ↓
...
- Verify the IAM trust policy above has the correct repo (`AVI5211/test`)
...
- If stuck: `argocd app sync test` manually
